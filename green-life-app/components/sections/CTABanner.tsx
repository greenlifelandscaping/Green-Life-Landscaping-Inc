...
const variantStyles: Record<Variant, { bg: string; accent: string; whiteBtnText: string }> = {
  green: {
    bg: 'bg-brand-primary',
    accent: 'text-brand-accent',
    whiteBtnText: 'text-brand-primary',
  },
  snow: {
    bg: 'bg-haint-700',
    accent: 'text-haint-300',
    whiteBtnText: 'text-haint-700',
  },
};
  ...
  const renderedTitle =
    title ?? (
      <>
        ...
        <br />
        <em className={`not-italic ${v.accent}`}>proud to come home to?</em>
      </>
    );
  return (
    <section className={v.bg}>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 lg:gap-8 text-center lg:text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-3.5 lg:flex-shrink-0 lg:items-center justify-center">
            <Link
              ...
              className={`inline-flex items-center justify-center gap-2 bg-white ${v.whiteBtnText} font-display font-bold text-fluid-cta tracking-[0.03em] px-6 sm:px-8 py-3.5 sm:py-4 rounded-md shadow-card hover:bg-surface-alt hover:-translate-y-0.5 transition-all`}
            >
