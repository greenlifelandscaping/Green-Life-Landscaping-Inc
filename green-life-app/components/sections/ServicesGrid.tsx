      ...
      <div className="container-page">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
          {SERVICES.map((service, idx) => {
            ...
            const lastSpan = SERVICES.length === 5 && idx === 4 ? 'md:col-span-3 lg:col-span-1' : '';
            return (
              <Link
                key={service.slug}
                href={`/services/${service.slug}`}
                className={`group flex flex-col bg-white rounded-lg shadow-card transition-all duration-250 ease-out p-7 hover:shadow-card-hover hover:-translate-y-0.5 ${
                  isSnow
